...
static MemRefType convertTensorToMemRef(TensorType type) {
  ...
  //index_old = i * dim_1 + j
  AffineExpr index = mlir::getAffineConstantExpr(0, context);
  for(int i = 0; i < type.getRank(); i++) {
    AffineExpr currIndex = mlir::getAffineDimExpr(i, context);

    for(int j = i + 1; j < type.getRank(); j++) {
      currIndex = mlir::getAffineBinaryOpExpr(
          AffineExprKind::Mul,
          currIndex,
          mlir::getAffineConstantExpr(type.getShape()[j], context)
      );
    }

    index = mlir::getAffineBinaryOpExpr(
        AffineExprKind::Add,
        index,
        currIndex
    );
...
struct TensorSliceLowering: public ConversionPattern {
  TensorSliceLowering(MLIRContext *ctx)
  : ConversionPattern(toy::TensorSliceOp::getOperationName(), 1, ctx) {}
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,  ConversionPatternRewriter &rewriter) const final {
    ...
    ArrayAttr lower = cast<toy::TensorSliceOp>(op).lower();
    size_t dimensions = lower.size();

    std::vector<AffineMap> maps;
    maps.reserve(dimensions);
    ...
    for(size_t i = 0; i < dimensions; i++) {
      auto affineMap = AffineMap::get(
          ...
          mlir::getAffineBinaryOpExpr(
              AffineExprKind::Add,
              mlir::getAffineDimExpr(0, getContext()),
              mlir::getAffineConstantExpr(lower[i].cast<IntegerAttr>().getInt(), getContext())
              )
          );
      maps.push_back(affineMap);
    }
    ...
    lowerOpToLoops(op, operands, rewriter,
                   [loc, maps](
                     ...
                     // Generate an adaptor for the remapped operands of the
                     // TensorSliceOp. This allows for using the nice named
                     // accessors that are generated by the ODS.
                     toy::TensorSliceOpAdaptor sliceOpAdaptor(memRefOperands);

                     Value input = sliceOpAdaptor.input();

                     SmallVector<Value, 2> sliceIvs;
                     sliceIvs.reserve(loopIvs.size());

                     for(size_t i = 0; i < loopIvs.size(); i++) {
                       sliceIvs.push_back(
                           builder.create<AffineApplyOp>(loc, maps[i], llvm::makeArrayRef({ loopIvs[i] }))
                       );
                     }

                     return builder.create<AffineLoadOp>(loc, input, sliceIvs);
                   });
    return success();
  }
};
...
void ToyToAffineLoweringPass::runOnFunction() {
  ...
  patterns.add<AddOpLowering, ConstantOpLowering, MulOpLowering,
               ReturnOpLowering, TransposeOpLowering, TensorSliceLowering>(&getContext());
