...
/// Lowers `toy.printf` to a loop nest calling `printf` on each of the individual
/// elements of the array.
class PrintfOpLowering : public ConversionPattern {
public:
  explicit PrintfOpLowering(MLIRContext *context)
      : ConversionPattern(toy::PrintfOp::getOperationName(), 1, context) {}
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    auto printfOp = cast<toy::PrintfOp>(op);
    rewriter.create<CallOp>(
        loc, printfRef, rewriter.getIntegerType(32),
        ArrayRef<Value>({formatSpecifierCst, printfOp.input()}));
...
/// elements of the array.
class PrintNewLineLowering : public ConversionPattern {
public:
  explicit PrintNewLineLowering(MLIRContext *context)
      : ConversionPattern(toy::PrintNewLineOp::getOperationName(), 1, context) {}
...
void ToyToLLVMLoweringPass::runOnOperation() {
  ...
  // PrintOp.
  patterns.add<PrintNewLineLowering, PrintfOpLowering>(&getContext());
